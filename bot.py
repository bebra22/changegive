...
def start(message):
	...
	bot.send_message(message.chat.id, '''👋 Привет! 👋
		...
		Этот бот для накрутки гемов и уровней в гача лайф
...
def start(message):
	...
	first_button = types.InlineKeyboardButton(text="кристалов", callback_data="first")

	second_button = types.InlineKeyboardButton(text="уровни", callback_data="second")
...
def callback_inline(call):
	...
	if call.data == "first":
		...
		msg = bot.send_message(call.message.chat.id, 'Введите колличество кристалов  (не более 100000)') 
		...
		msg = bot.send_message(call.message.chat.id, 'Введите колличество подписчиков (не более 999)') 
...
def proc1(message):
	...
	try:
		...
		if int(num) > 100000:
			...
			bot.reply_to(message, 'Колличество кристалов  не может быть более 100000!')
...
def proc2(message):
	...
	try:
		...
		if int(num) > 999:
			...
			bot.reply_to(message, 'Колличество уровней не может быть более 999!')
